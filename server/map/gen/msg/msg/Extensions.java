...
public class Extensions {
          public static String tostring_msg_map_SMapDebugInfo(msg.map.SMapDebugInfo x) {
                ...
                return x.toString();
          } public static String tostring_list_msg_gmap_MapInfo(java.util.List<msg.gmap.MapInfo> x) {
                StringBuilder _s = new StringBuilder(); _s.append('[');
                for(var _e : x) {
                _s.append(msg.Extensions.tostring_msg_gmap_MapInfo(_e)).append(',');
        ...
        public static void marshal_msg_map_SMapDebugInfo(msg.map.SMapDebugInfo x, pcore.marshal.Octets os) {
            ...
            x.marshal(os);
        } public static void marshal_list_msg_gmap_MapInfo(java.util.List<msg.gmap.MapInfo> x, pcore.marshal.Octets os) {
            os.writeSize(x.size());
            for(var _e : x) {
            msg.Extensions.marshal_msg_gmap_MapInfo(_e,os);
            }
        ...
        } public static java.util.List<msg.gmap.MapInfo> unmarshal_list_msg_gmap_MapInfo(pcore.marshal.Octets os) {
            var n = Math.min(os.readSize(), os.size() + 1);
            java.util.List<msg.gmap.MapInfo> x = pcore.collection.Factory.newList(n);
            for(int i = 0 ; i < n ; i++) {
            x.add(msg.Extensions.unmarshal_msg_gmap_MapInfo(os));
            }return x;
        ...
        public static void marshal_compatible_msg_map_SMapDebugInfo(msg.map.SMapDebugInfo x, pcore.marshal.Octets os) {
            ...
            x.marshalCompatible(os);
        } public static void marshal_compatible_list_msg_gmap_MapInfo(java.util.List<msg.gmap.MapInfo> x, pcore.marshal.Octets os) {
            os.writeSize(x.size());
            for(var _e : x) {
            msg.Extensions.marshal_compatible_msg_gmap_MapInfo(_e,os);
            }
        ...
        } public static java.util.List<msg.gmap.MapInfo> unmarshal_compatible_list_msg_gmap_MapInfo(pcore.marshal.Octets os) {
            var n = Math.min(os.readSize(), os.size() + 1);
            java.util.List<msg.gmap.MapInfo> x = pcore.collection.Factory.newList(n);
            for(int i = 0 ; i < n ; i++) {
            x.add(msg.Extensions.unmarshal_compatible_msg_gmap_MapInfo(os));
            }return x;
